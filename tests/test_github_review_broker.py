...
class Client:
    def __init__(self, head=HEAD, checks=None, author="contributor"):
        self.head, self.checks, self.author, self.calls = (
            head,
            checks or {"tests": "success"},
            author,
            [],
        )
    ...
    def required_checks(self, repository, sha):
        return self.checks
...
def test_success_and_redaction(tmp_path):
    c = Client()
    result = broker(tmp_path, c).submit_review(req())
...
def test_rejections(tmp_path):
    for bad in (
        ...
        dict(preflight_head="d" * 40),
        dict(preflight_state="FAIL"),
        dict(preflight_checks=("tests",)),
        dict(decision="merge"),
        dict(body="please push"),
    ):
        with pytest.raises(BrokerError):
            broker(tmp_path).submit_review(req(**bad))
    with pytest.raises(BrokerError):
        broker(tmp_path, Client(checks={"tests": "failure"})).submit_review(
            req(request_id="failed")
        )
    with pytest.raises(BrokerError):
        broker(tmp_path, Client(head="c" * 40)).submit_review(req(request_id="drift"))
...
def test_timeout_is_audited(tmp_path):
    ...
    with pytest.raises(BrokerError, match="timed out"):
        broker(tmp_path, Timeout()).submit_review(req())
    assert '"exit_status":124' in (tmp_path / "audit.jsonl").read_text()


def test_malformed_input(tmp_path):
    with pytest.raises(BrokerError):
        broker(tmp_path).submit_review(req(pull_request=0))
